...
func updateTableSummary(ctx context.Context, db *sql.DB, instanceID, schema, table, checkpointSchemaName string, summaryInfo *tableSummaryInfo) error {
	if summaryInfo == nil {
		return nil
	}
...
func (t *TableDiff) setConfigHash() error {
	...
	log.Debug("tichecker config", zap.ByteString("config", jsonBytes), zap.String("hash", t.configHash))

	return nil
}

// createCheckpointTable creates checkpoint tables, include `summary` and `chunk`
func createCheckpointTable(ctx context.Context, db *sql.DB, checkpointSchemaName string) error {
...
func loadFromCheckPoint(ctx context.Context, db *sql.DB, schema, table, configHash, checkpointSchemaName string) (bool, error) {
...
func loadChunks(ctx context.Context, db *sql.DB, instanceID, schema, table, checkpointSchemaName string) ([]*ChunkRange, error) {
...
func cleanCheckpoint(ctx context.Context, db *sql.DB, schema, table, checkpointSchemaName string) error {
...
func initTableSummary(ctx context.Context, db *sql.DB, schema, table string, configHash, checkpointSchemaName string) error {
...
func initChunks(ctx context.Context, db *sql.DB, instanceID, schema, table, checkpointSchemaName string, chunks []*ChunkRange) error {
	beginTime := time.Now()
	batch := 100
	num := 0
...
func saveChunk(ctx context.Context, db *sql.DB, chunkID int, instanceID, schema, table, checksum, checkpointSchemaName string, chunk *ChunkRange) error {
	chunkBytes, err := json.Marshal(chunk)
	if err != nil {
		return errors.Trace(err)
	}
