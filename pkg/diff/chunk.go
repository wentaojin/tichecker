...
func SplitChunks(ctx context.Context,
	checkpointSchemaName string,
	table *TableInstance,
	splitFields, limits string,
	chunkSize int,
	collation string,
	useTiDBStatsInfo bool,
	cpDB *sql.DB) (chunks []*ChunkRange, err error) {
	...
	// init chunks -> insert into chunk table
	err = initChunks(ctx1, cpDB, table.InstanceID, table.Schema, table.Table, checkpointSchemaName, chunks)
	if err != nil {
		return nil, errors.Trace(err)
	}
...
func getChunksForTable(table *TableInstance, columns []*model.ColumnInfo, chunkSize int, limits string, collation string, useTiDBStatsInfo bool) ([]*ChunkRange, error) {
	...
	log.Info("get and split chunks", zap.Duration("cost", time.Since(beginTime)))
	return chunks, err
}
...
func (s *bucketSpliter) split(table *TableInstance, columns []*model.ColumnInfo, chunkSize int, limits string, collation string) ([]*ChunkRange, error) {
	...
	log.Info("get buckets", zap.Duration("cost", time.Since(beginTime)))
...
func (s *bucketSpliter) getChunksByBuckets() (chunks []*ChunkRange, err error) {
	...
	log.Info("get chunks by buckets", zap.Duration("cost", time.Since(beginTime)))
